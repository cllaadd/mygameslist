...
const GameDetail = () => {
  ...
  const [showMoreAlternativeNames, setShowMoreAlternativeNames] =
    useState(false);
  ...
  const visibleKeywords = showMoreKeywords
    ? keywords.slice(0, showLimitKeywords)
    : keywords.slice(0, 10);
  const visibleAlternativeNames = showMoreAlternativeNames
    ? altnames.slice(0, showLimitAlternativeNames)
    : altnames.slice(0, 5);
  return (
    <div>
          ...
          return (
            <div key={game.id}>
                      ...
                      <p>
                        ...
                        {game.platforms_id.map((platform, index) => (
                          <a
                            key={index}
                            className="platforms"
                            href={`/games/idsearch?search_param=Platform&search_param_name=${platform.name}&query_param=platforms_id&param_id=${platform._id}`}
                          >
                            {platform.name}
                          </a>
                        ))}
                  ...
                  {keywords.length > 10 && (
                    <button
                      ...
                      onClick={() => {
                        ...
                        if (!showMoreAlternativeNames)
                          setShowLimitAlternativeNames(100);
                  ...
                  {visibleKeywords.map((keyword, index) => (
                    <div key={index} className="keyword-button-container">
                      <a
                        ...
                        href={`/games/idsearch?search_param=Keywords&search_param_name=${keyword.name}&query_param=keywords_id&param_id=${keyword.id}`}
                      >
                        {keyword.name}
                      </a>
                    </div>
              ...
              <div className="similar-game-container">
                <h4>Similar Games</h4>
                <div
                  className="similar-game-carousel"
                  style={{ maxWidth: 420 }}
                >
                  <Carousel>
