    ...
    for platform in platform_db:
        if collection.find_one({"name": platform["name"]}) == None:
            rec_id1 = collection.insert_one(
                {"_id": platform["id"], "name": platform["name"]}
            )
        else:
            pass
...
def create_perspective_db():
    ...
    for perspective in perspective_db:
        if collection.find_one({"name": perspective["name"]}) == None:
            rec_id1 = collection.insert_one(
                {"_id": perspective["id"], "name": perspective["name"]}
            )
        else:
            pass
...
def find_collections_data(collection_offset: int):
    ...
    payload = "fields name,games;" "limit 500;" f"offset {collection_offset};"
    fields = (
        "id",
        "name",
        "games",
...
def find_franchises_data(franchise_offset: int):
    ...
    payload = "fields name,games;" "limit 500;" f"offset {franchise_offset};"
    fields = (
        "id",
        "name",
        "games",
...
def find_game_engines_data(engine_offset: int):
    ...
    payload = "fields name;" "limit 500;" f"offset {engine_offset};"
    fields = (
        "id",
        "name",
    )
        ...
        for game_engine in game_engines:
            ...
            try:
                ...
                if collection.find_one({"_id": game_engines_data["_id"]}) == None:
                    ...
                    print(
                        game_engines[game_engines_found]["_id"] + " could not be found"
                    )
            except Exception as e:
                pass
...
def find_keywords_data(keywords_offset: int):
    ...
    payload = "fields name;" "limit 500;" f"offset {keywords_offset};"
    fields = (
        "id",
        "name",
    )
...
def find_companies_data(company_offset: int):
    ...
    payload = "fields logo.image_id,*;" "limit 500;" f"offset {company_offset};"
    response = requests.request("POST", url, headers=header, data=payload)
    company_fields = (
        "id",
        "name",
                ...
                if field == "id":
                    ...
                    try:
                        logo_url = (
                            ...
                            + companies[company_number][field]["image_id"]
                            + ".png"
                        )
                    ...
                    try:
                        full_company_data["description"] = companies[company_number][
                            field
                        ]
                    except Exception as e:
...
def find_game_data(game_offset: int):
    ...
    payload = f"fields {fields_api};" "limit 500;" f"offset {game_offset};"
                ...
                if field == "id":
                    ...
                    try:
                        cover_url = (
                            ...
                            + games[game_number][field]["image_id"]
                            + ".jpg"
                        )
                    ...
                    try:
                        full_game_data["category"] = category_dict[
                    ...
                    try:
                        full_game_data["collection_id"] = games[game_number][field][
                            "id"
                        ]
                    except Exception as e:
                    ...
                    try:
                        ...
                        decoded_date = (
                            datetime.fromtimestamp(unix_timestamp) - timedelta(hours=2)
                        ).strftime("%B %d, %Y")
                    ...
                    try:
                        ...
                        for involved_company in involved_companies:
                            involved_companies_list.append(
                                involved_company["company"]["id"]
                            )
                        full_game_data[
                    ...
                    try:
                        if len(games[game_number][field]) > 0:
                            ...
                            if length_of_field <= 5:
                                similar_games = games[game_number][field][
                                    0:length_of_field
                                ]
                            else:
